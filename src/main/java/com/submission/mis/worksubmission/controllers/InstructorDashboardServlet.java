...
//             int studentCounts = 0;
//            for (Course course : instructor.getCourses()) {
//                int studentCount = hibernateSession.createQuery(
...
//}
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try (Session hibernateSession = sessionFactory.openSession()) {
            ...
            int studentCounts = hibernateSession.createQuery(
                            "SELECT COUNT(s) FROM Student s", Long.class)
                    .uniqueResult()
                    .intValue();

            request.setAttribute("user", instructor); // Pass the instructor to the JSP
            request.setAttribute("studentCounts", studentCounts); // Pass the total student count
            request.setAttribute("assignments", assignmentService.getAssignmentsByInstructor(instructor.getId()));
        }
